...
class BasicCoursesScreen extends StatefulWidget {
  const BasicCoursesScreen({super.key});

  @override
  State<BasicCoursesScreen> createState() => _BasicCoursesScreenState();
}

class _BasicCoursesScreenState extends State<BasicCoursesScreen> {
  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      Provider.of<ProfileProvider>(context, listen: false).loadProfile();
      // Загружаем курсы категории 'recycling' для основных курсов
      Provider.of<EducationProvider>(context, listen: false).fetchCourses(category: 'recycling');
    });
  }
  ...
  Widget _buildCourseCard(
    ...
    return GestureDetector(
      onTap: () {
        print('BasicCoursesScreen: Clicked course: ${course.title} (ID: ${course.id})');
      ...
      child: IntrinsicHeight(
        child: Container(
          ...
          child: Stack(
            children: [
              // Верхняя и нижняя части - верхняя растягивается больше
              Column(
                children: [
                  // Верхняя часть - название (растягивается под контент)
                  Flexible(
                    flex: 2, // Верхняя часть в 2 раза больше нижней
                    child: Container(
                      padding: EdgeInsets.only(
                        left: 16 * scaleWidth,
                  ...
                  // Нижняя часть - описание (фиксированная высота)
                  Flexible(
                    flex: 1,
                    child: Container(
                      padding: EdgeInsets.only(
              ...
              // Иконка Play наложенная на линию
              Positioned(
                right: 16 * scaleWidth,
                top: 0,
                bottom: 0,
